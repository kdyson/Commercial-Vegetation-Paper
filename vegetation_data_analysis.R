...
## Source data + helper R script
    ...
    source("vegetation_data_processing.R")
...
descriptive.stats.site <- tibble(
    ...
    tree.conifer.nat.dens = tree.conifer.native / sample.covariates$acres,
...
management.landscaping$tree.conifer.nat.dens <- descriptive.stats.site$tree.conifer.nat.dens
        ...
        temp <- summarise_all(descriptive.stats.site[ , 2:35],
                              funs(min, max, mean, sd, median)) %>%
            round(2)
        pretty.descriptive.stats.site <- tibble(
                    Metric = colnames(descriptive.stats.site)[2:35],
                    Minimum = unlist(temp[1:34]),
                    Maximum = unlist(temp[35:68]),
                    Mean = unlist(temp[69:102]),
                    `Standard Deviation` = unlist(temp[103:136]),
                    Median = unlist(temp[137:170])
        ...
        pretty.descriptive.stats.site$Metric <- c(
            ...
            "Tree Shannon Diversity (H')",
            "Tree Effective Species Richness (exp(H'))",
            ...
            "Native Conifer Density (#/acre)",
            "Native Conifer Abundance Large Trees Only",
            "Native Conifer Density Large Trees Only  (#/acre)",
            "Native Confier Abundance Medium & Large Trees",
            "Native Conifer Density Medium & Large Trees (#/acre)",
            ...
            "Shrub Shannon Diversity (H')",
            "Shrub Effective Species Richness (exp(H'))",
...
## -- PERMANOVA --------------------------------------------------------------------
    ...
    # Create summary tables of PERMANOVA/AICc results
        ...
        shrub.model.summary$Model <-  c("Tree Group Membership",
                                        ...
                                        #"Dead Wood (cube root)",
                                        "Tree Group + Median DF Height (m)",
                                        "Tree Group + Stands Predate Development",
                                        "Tree Group + Native Conifer Density",
                                        #"Tree Group + Dead Wood (cube root)",
                                        "Median DF Height (m) + Stands Predate Development",
                                        "Median DF Height (m) + Native Conifer Density",
...
save.image(file = paste0("vegetation_data_analysis_complete_", date, ".RData"))
